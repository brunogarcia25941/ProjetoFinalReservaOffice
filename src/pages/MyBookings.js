...
function MyBookings() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/resources`, { headers: { Authorization: `Bearer ${token}` } })
    ...
    toast(({ closeToast }) => (
      <div className="flex flex-col">
        ...
        <div className="flex gap-2">
          <button onClick={() => { efetuarCancelamento(); closeToast(); }} className="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-3 rounded-lg text-sm transition-colors">Sim, Cancelar</button>
  ...
  const atualizarReserva = async (e) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/bookings/${reservaEditando.booking_id}`, { resource_id: reservaEditando.resource_id, start_time: startFormatado, end_time: endFormatado }, { headers: { Authorization: `Bearer ${token}` } });
      toast.success("Reserva atualizada com sucesso!");
      setIsEditModalOpen(false);
      const response = await axios.get(`${API_URL}/bookings`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const eventosCalendario = reservas
    ...
    .map(r => ({
      ...
      borderColor: '#1d4ed8',
      textColor: '#ffffff',
      extendedProps: { nomeRecurso: r.resource_name }
    }));
  ...
  const handleEventClick = (clickInfo) => {
    ...
    toast(({ closeToast }) => (
      <div className="flex flex-col">
        ...
        <div className="flex gap-2">
          <button onClick={() => { closeToast(); abrirModalEdicao(idReserva); }} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 rounded-lg text-sm transition-colors">Editar</button>
          <button onClick={() => { closeToast(); cancelarReserva(idReserva, nomeDoRecurso); }} className="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-2 rounded-lg text-sm transition-colors">Cancelar</button>
        </div>
